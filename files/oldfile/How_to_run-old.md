...
root@master1:~# ansible-config init --disabled > /etc/ansible/ansible.cfg
...
# 并在安装完以后确认，如果是vmware exsi虚拟平台，不用改，保持默认即可
...
spec:
  ...
  natOutgoing: true
  nodeSelector: all()
  vxlanMode: CrossSubnet   （如果这里是这个需要修改成Always，kubectl edit installation default 修改encapsulation: VXLAN，改两个地方。并执行kubectl -n calico-system rollout restart ds calico-node）
